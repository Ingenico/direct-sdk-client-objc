...
@implementation IDPaymentRequest
...
- (NSString *)valueForField:(NSString *)paymentProductFieldId
{
    ...
    if (value == nil) {
        value = @"";
        IDPaymentProductField *field = [self.paymentProduct paymentProductFieldWithId:paymentProductFieldId];
        for (IDValidator *validator in field.dataRestrictions.validators.validators) {
            if ([validator class] == [IDValidatorFixedList class]) {
...
- (void)validate
{
    if (self.paymentProduct == nil || ![self.paymentProduct isKindOfClass:[IDPaymentProduct class]]) {
        [NSException raise:@"Invalid payment product" format:@"Payment product is invalid"];
    }
    
    [self.errors removeAllObjects];
    for (IDPaymentProductField *field in self.paymentProduct.fields.paymentProductFields) {
